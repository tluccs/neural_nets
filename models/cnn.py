...
class CNN:
    def __init__(self, X_train, y_train, X_val, y_val, X_test, y_test, start=0, end=1000, downsample=1, channel_pos='channels_last'):
    ...
    def train(self, stride=2, optimizer='adam', epochs=5):
        self.model.add(keras.layers.Conv1D(300, 5, strides=stride, padding='same', data_format=self.channel_pos, use_bias=True))
        self.model.add(keras.layers.Activation('relu'))
        self.model.add(keras.layers.MaxPooling1D(pool_size=2, strides=None, padding='same', data_format=self.channel_pos))
        self.model.add(keras.layers.Activation('relu'))
        self.model.add(keras.layers.Conv1D(50, 3, strides=stride, padding='same', data_format=self.channel_pos, use_bias=True))
        self.model.add(keras.layers.Activation('relu'))
        self.model.add(keras.layers.MaxPooling1D(pool_size=2, strides=None, padding='same', data_format=self.channel_pos))
        self.model.add(keras.layers.Activation('relu'))
